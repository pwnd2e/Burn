...
@interface KWAudioDiscInspector()
...
- (void)awakeFromNib
{
    ...
    NSImage *iconImage = [[NSWorkspace sharedWorkspace] iconForFileType:NSFileTypeForHFSTypeCode(kGenericCDROMIcon)];
...
- (void)updateView:(id)object
{
    ...
    while ((cntl = [iter nextObject]) != NULL)
    {
	    ...
	    {    
    	    ...
    	    {
	    	    NSString *string = [[NSString alloc] initWithData:property encoding:NSASCIIStringEncoding];
